...
namespace RFDevice {
	...
	class RFDEVICE_API RF25xDevice : public RFComDevice
	{
	public:	
		RF25xDevice();
		~RF25xDevice();
		...
		BOOL	HelloCmd( BYTE, LPRF25xHELLOANSWER );	
		BOOL	HelloCmd();	
		...
		BOOL	GetStreamMeasures(LPRF60xSTREAMVALUE lpusStreamValues, DWORD dwMeasuresCount, LPDWORD lpdwReadedMeasuresCount, LPDWORD lpdwBrokenPacketsCount);
		BOOL	StartStream();
		BOOL	StopStream();
	...
	protected:
		...
		LPRF25xHELLOANSWER m_helloAnswer;
