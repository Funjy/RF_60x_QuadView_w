...
namespace RFDevice {
	...
	class RFDEVICE_API RF60xDevice : public RFComDevice
	{
	public:
		//-----------------------------------------------------------------------------
		...
		//-----------------------------------------------------------------------------
		//  FUNCTION  RF60xDevice::OpenPort
		/// \brief    Open Serial port with lpPort_Name name and dwSpeed speed.
		///           
		...
		/// \return  BOOL : TRUE on success, FALSE on error.
		//-----------------------------------------------------------------------------
		BOOL	HelloCmd(BYTE bAddress, LPRF60xHELLOANSWER lprfHelloAnswer);
		
		//-----------------------------------------------------------------------------
		//  FUNCTION  RF60xDevice::HelloCmd
		/// \brief    Send HelloCmd to device and capture the answer.
		///           
		/// \return   BOOL : TRUE on success, FALSE on error.
		//-----------------------------------------------------------------------------
		BOOL	HelloCmd();
		...
		/// \return    : 
		//-----------------------------------------------------------------------------
		LPRF60xPARAMS GetParams();

	protected:
		RF60xPARAMS m_params;	
		USHORT	m_lastCapturedUnnormalizedMeasure;
		float	m_lastCapturedMeasure;
		RF60xHELLOANSWER m_helloAnswer;
